...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let json_file = matches.value_of("variables_json").unwrap_or("");
    ...
    if let Err(err) = manifest_writer(output_file, &rendered_template) {
        eprintln!("Error: {}", err);
...
fn manifest_writer(output: &str, template: &String) -> Result<(), Box<dyn std::error::Error>> {
    if output.is_empty() {
